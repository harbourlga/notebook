...
4.正常关闭数据库的时候，会把内存中所有的脏页都flush到磁盘上。
...
1⃣️背景：业务中经常需要随机展示数据库的一些信息，应该避免使用order by rand()来实现实时返回任务。
...
​     2.等flush:通常对一张表或全部表做flush操作很快就执行完，假如给堵住了通常是因为前面还有一个线程在占据着表导致flush操作没办法去关闭表，从而flush就会堵住后面的查询了。
...
1⃣️背景：在业务高峰期，生产环境的mysql压力过大，没法正常响应，需要短期内、临时性地提升一些性能
...
​     (5)GTID生成方式：有两种生成方式，取决于session变量gtid_next的值
...
​    以上流程表明了，mysql是“边读边发”。客户端只有接收了服务端发送的数据，服务端才会继续去读数据写入net_buffer。如果客户端接收很慢或者堵住了，那么会导致服务端由于结果发不出去，导致这个事务执行时间变长。事务执行时间变长，会导致这个事务占据锁的那些数据不能给别的请求去利用。

​    所以对于线上生产环境，对于查询结果都建议先load到本地内存然后再做后续的数据处理，不要占用服务端的资源去计算。如果因为大查询导致线上生产环境内存压力太大，这时候应该适当的让服务端别一次性返回那么多数据，分批去处理。
...
1⃣️被驱动表使用了索引的情况(Index Nested-Loop Join)
...
​    假如不使用join执行以上t1,t2过程，扫描行数也是一样的。但是需要多执行很多条语句

3.怎么选择驱动表

   以上t1和t2过程, 驱动表走的是全表扫描(时间复杂度o(n))，被驱动表走的是树搜索(时间复杂度O(n*2*logm))。整个join的快慢受驱动表的影响，因此驱动表应该选择小表。

4.总结

   a.使用join语句，性能比拆开多个单表执行sql语句的性能要好

   b.使用join语句，驱动表要选择小表

   c.以上结论的前提是，join的时候是使用被驱动表的索引

2⃣️被驱动表没有使用索引(Simple Nested-Loop Join)

   假如被驱动表没有索引，那每次驱动表取一行数据拿字段去被驱动表匹配的时候都要进行全表扫描那是无法接受的。

   因此mysql针对这种情况没有使用Simple Nested-Loop Join算法，而是使用另外一个算法Block Nested-Loop Join,简称BNL.

3⃣️Block Nested-Loop Join

   核心是通过将数据写到内存来提高效率。

   join_buffer：存入驱动表的数据，如果一次性放不完则分多段存储。扫描被驱动表的数据，与join_buffer中的数据做对比，满足join条件的放进结果集返回

   总是应该使用小表作为驱动表。

4⃣️总结

   **1.如果可以使用被驱动表的索引，join语句是有优势的。**

   2.不能使用被驱动表的索引，只能使用Block Nested-Loop Join算法，这样的语句尽量不使用
...
​      背景：回表通常是在普通索引上查找到主键id之后再到主键索引上一行行的去查找。如果随着普通索引的值递增顺序查询的话，主键id就变成随机访问的了。如果可以调整整个查询的顺序，让主键id递增排序然后再到主键索引上查询此时就能够加速了。

​      流程：a.根据普通索引a，定位到满足条件的记录，将所有主键id放入到read_rnd_buffer。

​                 b.将read_rnd_buffer中的id进行递增排序

​                 c.排序后的id数组，一次到主键id索引中查记录，并作为结果返回

​      总结：MRR能够提升性能的核心在于，这条查询语句在普通索引a上做的是一个范围查询，可以得到足够多的主键id值。对这些id通过排序之后，再去主键索引查数据，这时候才能够体现“顺序性”的优势

   (2)Batched Key Access(BKA)

​       背景：该算法是对Index Nested-Loop Join的优化，通过将驱动表查到的普通索引a利用MRR进行优化

​       流程: 利用join_buffer一次性将驱动表放入内存中传给被驱动表。

​       启动方式：set optimizer_switch='mrr=on,mrr_cost_based=off,batched_key_access=on'
...
​           a.可能会多次扫描被驱动表，占用磁盘io资源

​           b.判断join条件需要执行M*N次对比(M和N分别是两张表的行数)，如果是大表就会占用非常多的cpu资源。

​           c.可能会导致Buffer Pool的热数据被淘汰，影响内存命中率。

​       2.如果通过分析和explain语句确认使用了BNL算法，那必须就要进行优化。优化的常见做法是，给被驱动表的join字段加上索引，把BNL算法转成BKA算法。

​       3.BNL算法转成BKA算法

​          a.可以直接在被驱动表上建索引，这时就可以直接转成BKA算法

​          b.对于不方便在被驱动表上建立索引且被驱动表查询行数很少的情况下，可以将被驱动表放到临时表，然后对临时表建立索引来触发BKA算法

​          **c.直接把数据读出来加载到业务端。通过建立一个驱动表的hash表，扫描被驱动表加载到本地的行数据，一行行的去hash表匹配结果。该过程会比临时表方案更快。**
...
​    1.内存表：指的是在建表的时候使用Memory引擎的表。表的数据都保存在内存里，系统重启的时候就会被清空，但表结构还在。

​    2.临时表：可以使用各种引擎类型。如果是使用InnoDB引擎或者MyISAM引擎的临时表，写数据的时候是写到磁盘上。临时表也可以使用memory引擎。

2⃣️临时表的特点

​    1.建表语句是create temporary table...

​    2.一个临时表只能被创建它的session访问，对其他线程不可见。

​    3.临时表可以与普通表同名

​    4.一个session内有同名的临时表和普通表的时候，show create 语句，以及增删查改语句访问的是临时表。
...
​         例如select v from ht where f=N;

​             假如拆了1024个表，那么插入数据的时候根据分表规则(最简单的一种方式N%1024)插入到指定的表上。查询的时候通过分表规则来确认数据被放到那个分表上面。

​       (2).第二种情况是查询条件为“分区key”之外的字段，此时无法通过分区字段来直接定位到指定表。

​          例如select v from ht where k>=M order by t_modified desc limit 100; 在分区表中查找满足条件所有行，然后统一做order by操作
...
​               缺点：实现逻辑复杂对中间层开发能力要求高；对proxy端压力大
...
​     执行临时表创建的时候，mysql会给这个临时表创建一个frm文件保存表结构定义。

​     该文件名的后缀是.frm，前缀是“#sql\_{process id}\_{thread id}\_序列号” 

​    **内存上：**

​     mysql维护数据表，除了物理磁盘上要有文件，内存里面也要有一套机制区别不同的表，**每个表都对应一个table_def_key**

​     普通表table_def_key的值是“库名+表名”

​     临时表table_def_key在"库名+表名"的基础上，加入了"server_id+thread_id"  

​    **实现上：**

​     每个线程维护了一个临时表链表，每次session内操作表的时候，先遍历链表看看是否有临时表有的话就优先操作临时表，没有才轮到普通表。

​      session结束时，执行drop回收临时表。并且会在binlog中执行目的是为了主备复制

 5⃣️临时表和主备复制    

  1.目的：临时表需要写到binlog是为了当出现普通表和临时表交互现象的时候，备库执行语句不至于报错。

  2.如果当前binlog_format=row，由于记录了操作的数据，那么跟临时表有关的语句是不会记录到binlog;也就是说，binlog_format=statment/mixed的时候,binlog才会记录临时表的操作。

  3.主库的临时表命名会传给备库，以保证备库执行的语句是在正确的表上面。

 6⃣️内部临时表

  1.背景

​       内部临时表与手动创建的临时表不同，它是mysql自发创建用来存放语句执行过程的中间数据，以辅助sql语句的执行。包括sort buffer(排序时使用)、join buffer(使用join语句时使用)、内存临时表。
...
​    group by的语义逻辑是统计不同的值出现的个数，但是每一行(id%100)求分组结果是无序的，所以我们需要一个临时表来记录并统计结果。如果提前做了一些准备使得扫描出现的数据是有序的那么就不需要临时表了。

​    通过**generated column**机制，在添加一个新列的时候让其强制关联上更新上groupby 的字段，并且对这个新列创建索引。那么这个新列就是有序的了。那么groupby语句的字段就可以用上这个新字段

​    例如：select id%100 as m, count(*) as c from t1 group by m;

​        转化成----> alter table t1 add column z int generated always as(id % 100), add index(z);

​                            select z, count(*) as c from t1 group by z;

   (2)直接在磁盘排序

​    在group by语句中加入SQL_BIG_RESULT，告诉优化器，语句涉及数据量大。

​    select SQL_BIG_RESULT id%100 as m, count(*) as c from t1 group by m;

​    a.初始化sort_buffer,确定放入一个整型字段，记为m；

​    b. 扫描表t1的索引a，依次取出id值，将id%100的值存入sort_buffer;

​    c.扫描完成后，对sort_buffer字段m做排序

​    d.排序完成后就得到了一个有序数组。根据有序数组，得到数组里面不同值及其出现的次数就是最终的结果。

***总结***：

​    (1)新版本的mysql group by不会主动排序，需要自己去排

​    (2)尽量让group by过程用上表的索引，确认方法是explain结果里没有Using temporary 和 Using filesort

​    (3)如果group by 需要统计的数据量不大，尽量只使用内存临时表；也可以通过适当调大tmp_table_size参数，避免用到磁盘临时表。

​    (4)如果数据量实在太大，使用SQL_BIG_RESULT这个提示，来告诉优化器直接用排序算法得到group by的结果。

​    

4.总结mysql什么时候会使用内部临时表

   (1)如果语句执行过程中可以一边读数据，一边直接得到结果，是不需要额外内存，否则就需要额外的内存来保存中间结果

   (2)join_buffer是无序数组，sort_buffer是有序数组，临时表是二维结构

   (3)如果执行逻辑需要用到二维表特性，就会优先考虑使用临时表。比如上面的例子，union需要用到唯一索引约束，group by需要用到另外一个字段来存累积计数。
...
​     Memory引擎：数据和索引是分开的
  ...
  (1)Innodb的数据总是有序存放的，而内存表的数据就是按照写入顺序存放的。

  (2)当数据文件有空洞的时候，Innodb位了保证数据有序性，只能在固定的位置插入新值。而内存表找到空位就可以插入。

  (3)数据位置发生变化的时候，Innodb只需要修改主键索引。而内存表需要修改所有的索引

  (4)Innodb用主键索引查询时需要走一次索引查找，用普通索引查询时候需要走两次索引查找。而内存表所有的索引地位都是一样的，一次查询就可以。

  (5)Innodb支持变长数据类型，不同记录的长度可能不同；而内存表每行数据长度相同。

  (6)内存表如果执行范围查询是用不上主键索引，需要走全表扫描
...
3⃣️内存表的锁
  ...
  1.背景：在生产环境上不建议使用内存表，主要有**锁粒度**和**数据持久化**问题

  2.表的锁粒度

   (1)内存表不支持行锁，只支持表锁。因此一张表只要更新，就会堵住其他所有在这个表上的读写操作。

   (2)由于只支持表锁，因此在处理并发事务的时候，性能不会太友好

  3.数据持久性

   (1)数据库重启的时候，内存表会丢失。在M-S架构下，备库发生重启，数据不见了，客户端发送更新请求进来会导致主备不一致报错。更极端的如果在双M结构下，由于主备不一致会启动主备切换，那么主库会跟备库一样做了一遍删除的操作导致数据全部丢失了。

4⃣️普通内存表的场景

  临时表不会被其他线程访问，没有并发性问题。

  临时表重启后也是需要删除的，不小心清空数据这个问题不存在
...
1⃣️自增值保存在哪里
   ...
   (2)Myisam引擎的自增值保存在数据文件中

​       InnoDB的自增值保存在了内存中。在较新的版本实现了自增值持久化的能力，发生重启可以恢复到重启前的值。

2⃣️自增值的修改机制

  (1)在插入一行数据的时候，自增值的行为如下：

​      a.如果插入数据时id字段为0、null或未指定值，那么就把这个表当前的自增值填到自增字段。

​      b.如果插入数据时id字段指定了具体的值，那么就直接使用语句里指定的值。

  (2)自增值大小和插入值大小的不同的行为不同：

​      a.如果自增值大于插入，那么这个表的自增值不变；

​      b.如果自增值小于等于插入值，那么就把当前自增值修改为新的插入值。

  (3)自增值的生成算法

​     auto_increment_offset:初始的自增值，默认为1

​     auto_increment_increment:自增值的步长，初始为1。在分表分库双写的情况下，可以设置id自增值步长为2，初始值1的库是奇数，初始值为2的是偶数，避免两个库发生主键冲突。

3⃣️自增值的修改时机

   (1)插入语句出现错误

​     这个语句的错误不是在分析器阶段词法语法错误。而是因为插入的语句没有指定自增id的值，将自增值赋给传入行，并且自增值+1之后出现了错误。通常是由于除了主键之外还设了一个唯一索引，在唯一索引扫描期间发现Duplicate key error导致插入报错语句返回。但此时自增值已经+1了，再次插入正确的语句就会出现自增主键不连续的情况。

   (2)事务回滚，已经增加的自增值不能回退

​      为了保证不同事务之间不会冲突，同时为了避免锁的粒度太大，InnoDB放弃了自增值回退的设计。只保证自增id是递增的，但不保证是连续的。

4⃣️自增锁的优化

​    innodb_autoinc_lock_mode：设置自增锁的的范围。设置为2时，所有申请自增主键的动作都是申请后就释放锁。

​    在生产环境上，将innodb_autoinc_lock_mode=2,并且binlog_format=row。这样既能提升并发性，又不会出现数据一致性的问题。



# insert语句的锁

1⃣️insert...select在什么情况下要锁全表

​    该语句是很常见的两个表之间拷贝数据的方法，在可重复读隔离级别下，这个语句会给select的表里扫描到的记录和间隙加读锁。
...
​     背景：当往自己表的末尾插入一行数据，该数据行来源于表自身某字段的最大值+1。即读自己的表的同时，把读出来的数据直接写回原表。

​     发生了什么？

​       在这种情况下，语句会在表上根据指定索引做全表扫描把数据放进临时表，然后在根据条件取临时表的数据插回到原表中。此时会导致索引上的所有间隙都加上锁，所以会导致其他事务不能在这个表上插入数据。

​     为什么这样设计？

​       一边遍历数据，一边更新数据，如果读出来的数据直接写回原表，就可能在遍历过程中，读到刚刚插入的记录，新插入的记录如果参与计算逻辑就跟语义不符。

3⃣️insert唯一键冲突

​     背景：在一个事务中，当执行insert语句发生唯一键冲突的时候，并不只是简单地报错返回，还在冲突的索引上加上了next-key lock。

​         如果没有冲突，则在该插入行的唯一索引上加上记录锁。

​     冲突发生场景：

​           (1)假如在另一个session插入行数据的唯一索引值在next-key lock上，那就会发生锁等待状态。

​           (2)还有一种可能就是两个session共同抢一个唯一索引值写数据的时候会造成死锁。

4⃣️insert into ... on duplicate key update
