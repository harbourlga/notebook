...
![image-20210513213943321](images/image-20210513213943321.png)
...
​     浏览器会在请求头里添加一个“Cache-Control: max-age=0"，告诉服务器生成最新的报文。

   (2)强制刷新时

​     发送了一个“Cache-Control: no-cache"跟刷新操作时一样请求最新的报文

   (3)点击前进、后退按钮时和重定向跳转

​     会发现状态码出现"from disk cache"字样，意思是没有发送网络请求。

​     在这些动作中，只会用最基本的请求头，没有"Cache-Control"，所以就会检查缓存，直接利用之前的资源。

4.条件请求

  背景：浏览器通过两个连续的请求(先发个head查看最新修改时间，然后与缓存比较，有改动则再发一次请求获取最新版本)进行验证缓存是否过期代价太高。通过加字段，让服务器判断是否要返回最新，这样一次请求就可以搞定了。

  逻辑：

​    (1)第一次响应报文给客户端浏览器发送一个资源的唯一标识实体标签或者最后修改时间

​     "ETag" :表示实体标签，是报文资源的唯一标识。携带这个字段可以让服务器判断客户端的资源是不是最新的。有强弱之分，强"ETag"必须保证资源在字节级别完全相符，弱的话部分相符即可。

​     "Last-modified":资源的最后修改时间。

​    (2)浏览器第二次请求

​     "if-Modified-Since": 传回上面的 "ETag" 向服务器询问是否有最新的

​     "Last-modified":传回上面的 "ETag"向服务器询问是否有最新的

​     "If-None-Match":传回上面弱 "ETag"或强 "ETag"请求较新的资源。

![image-20210519171413864](images/image-20210519171413864.png)